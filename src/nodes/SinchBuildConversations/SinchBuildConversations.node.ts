...
export class SinchBuildConversations implements INodeType {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    // For Send Message operation, check if we have multiple input items
    // This can cause duplicate messages if parameters are the same
    const resource = items.length > 0 ? this.getNodeParameter('resource', 0) as string : 'message';
    const operation = items.length > 0 ? this.getNodeParameter('operation', 0) as string : 'send';
    
    if (resource === 'message' && operation === 'send' && items.length > 1) {
      // Warn about multiple input items for Send Message
      // n8n will process each item separately, which can cause duplicate sends
      const firstTo = this.getNodeParameter('to', 0) as string;
      const firstMessage = this.getNodeParameter('message', 0) as string;
      
      // Check if all items have the same parameters
      let allSame = true;
      for (let i = 1; i < items.length; i++) {
        const to = this.getNodeParameter('to', i) as string;
        const msg = this.getNodeParameter('message', i) as string;
        if (to !== firstTo || msg !== firstMessage) {
          allSame = false;
          break;
        }
      }
      ...
      if (allSame) {
        // All items have the same parameters - only process the first one
        // This prevents duplicate messages when parameters are identical
        const toRaw = this.getNodeParameter('to', 0) as string;
        const defaultCountry = this.getNodeParameter('defaultCountry', 0, '') as string || undefined;
        const message = this.getNodeParameter('message', 0) as string;
        const additional = this.getNodeParameter('additionalFields', 0, {}) as {
          smsSender?: string;
          callbackUrl?: string;
          metadata?: string;
        };
